...
threadPool::threadPool(EventLoop *baseLoop, int numThreads)
    : baseLoop_(baseLoop), numThreads_(numThreads), is_started_(false),
      next_(0) { // numThreads为0的话退化成单线程
  ...
  if (!is_started_) {
    if (numThreads_ == 0) {
      ...
      for (int i = 0; i < numThreads_; i++) {
        ...
        // 创建worker线程，worker线程执行loop，并返回loop对象指针
        EventLoop *loop = thread->startLoop();

        // 加入 threads_ 和 loops_ 容器
        threads_.emplace_back(std::move(thread));
        loops_.emplace_back(loop);
      }
      LOG_DEBUG << "ThreadPool started with " << numThreads_ << " threads."
                << std::endl;
    }
    is_started_ = true;
  }
