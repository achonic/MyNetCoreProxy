...
EventLoopThread::EventLoopThread()
    :loop_ptr_(nullptr), exiting_(false){
...
EventLoop* EventLoopThread::startLoop(){
    // 这里建一个子线程的操作，是让子线程运行loop事件循环，执行任务
    // 这里主线程则将loop的地址返回给创建这个startloop的主线程Eventloop

    thread_ = std::thread(&EventLoopThread::threadFunc,this);
    // 加锁 同步互斥操作，mutex是对loop_ptr_原子操作，condition_variable_让notify来唤醒wait
    std::unique_lock<std::mutex> lock(mutex_);

    //条件变量互斥锁，第二个参数确保线程被唤醒后，共享条件确实满足，避免“虚假唤醒”
