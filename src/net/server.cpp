...
Server::Server(EventLoop* loop, const InetAddress& listenAddr)
    : loop_(loop),
      ...
      threadPool_(std::make_unique<threadPool>(loop, 4)),
      started_(0),
      mutex_()
    {
        acceptor_->setNewConnectCallback([this](int sockfd, const InetAddress& peerAddr) {
...
void Server::newConnection(int sockfd, const InetAddress& peerAddr) {
    LOG_DEBUG << "Server::newConnection - new connection [" << peerAddr.toIpPort() << "] fd=" << sockfd << std::endl;

    EventLoop* ioLoop = threadPool_->getNextLoop();
    std::string connName = "conn_" + std::to_string(sockfd);

    ioLoop->runInLoop([this, ioLoop, connName, sockfd, peerAddr]() {
        auto conn = std::make_unique<TcpConnection>(ioLoop, connName, sockfd, peerAddr);
        ...
        conn->setCloseCallback(
            [this](const TcpConnection* conn_ptr) {
                removeConnection(conn_ptr);
            });
...
void Server::removeConnection(const TcpConnection* conn) {
    LOG_DEBUG << "Server::removeConnection [" << conn->name() << "] - connection closed" << std::endl;
